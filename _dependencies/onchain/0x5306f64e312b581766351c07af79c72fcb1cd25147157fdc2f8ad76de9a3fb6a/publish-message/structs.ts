...
export function isWormholeMessage(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V1}::publish_message::WormholeMessage`; }
...
export class WormholeMessage implements StructClass { __StructClass = true as const;

 static readonly $typeName = `${PKG_V1}::publish_message::WormholeMessage`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

 readonly $typeName = WormholeMessage.$typeName; readonly $fullTypeName: `${typeof PKG_V1}::publish_message::WormholeMessage`; readonly $typeArgs: []; readonly $isPhantom = WormholeMessage.$isPhantom;
 ...
 private constructor(typeArgs: [], fields: WormholeMessageFields, ) { this.$fullTypeName = composeSuiType( WormholeMessage.$typeName, ...typeArgs ) as `${typeof PKG_V1}::publish_message::WormholeMessage`; this.$typeArgs = typeArgs;
 ...
 static reified( ): WormholeMessageReified { return { typeName: WormholeMessage.$typeName, fullTypeName: composeSuiType( WormholeMessage.$typeName, ...[] ) as `${typeof PKG_V1}::publish_message::WormholeMessage`, typeArgs: [ ] as [], isPhantom: WormholeMessage.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => WormholeMessage.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => WormholeMessage.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => WormholeMessage.fromBcs( data, ), bcs: WormholeMessage.bcs, fromJSONField: (field: any) => WormholeMessage.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => WormholeMessage.fromJSON( json, ), fromSuiParsedData: (content: SuiParsedData) => WormholeMessage.fromSuiParsedData( content, ), fromSuiObjectData: (content: SuiObjectData) => WormholeMessage.fromSuiObjectData( content, ), fetch: async (client: SuiClient, id: string) => WormholeMessage.fetch( client, id, ), new: ( fields: WormholeMessageFields, ) => { return new WormholeMessage( [], fields ) }, kind: "StructClassReified", } }
 ...
 static fromSuiObjectData( data: SuiObjectData ): WormholeMessage { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isWormholeMessage(data.bcs.type)) { throw new Error(`object at is not a WormholeMessage object`); }

 return WormholeMessage.fromBcs( fromB64(data.bcs.bcsBytes) ); } if (data.content) { return WormholeMessage.fromSuiParsedData( data.content ) } throw new Error( "Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request." ); }
...
export function isMessageTicket(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V1}::publish_message::MessageTicket`; }
...
export class MessageTicket implements StructClass { __StructClass = true as const;

 static readonly $typeName = `${PKG_V1}::publish_message::MessageTicket`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

 readonly $typeName = MessageTicket.$typeName; readonly $fullTypeName: `${typeof PKG_V1}::publish_message::MessageTicket`; readonly $typeArgs: []; readonly $isPhantom = MessageTicket.$isPhantom;

 readonly sender: ToField<ID>; readonly sequence: ToField<"u64">; readonly nonce: ToField<"u32">; readonly payload: ToField<Vector<"u8">>

 private constructor(typeArgs: [], fields: MessageTicketFields, ) { this.$fullTypeName = composeSuiType( MessageTicket.$typeName, ...typeArgs ) as `${typeof PKG_V1}::publish_message::MessageTicket`; this.$typeArgs = typeArgs;

 this.sender = fields.sender;; this.sequence = fields.sequence;; this.nonce = fields.nonce;; this.payload = fields.payload; }

 static reified( ): MessageTicketReified { return { typeName: MessageTicket.$typeName, fullTypeName: composeSuiType( MessageTicket.$typeName, ...[] ) as `${typeof PKG_V1}::publish_message::MessageTicket`, typeArgs: [ ] as [], isPhantom: MessageTicket.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => MessageTicket.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => MessageTicket.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => MessageTicket.fromBcs( data, ), bcs: MessageTicket.bcs, fromJSONField: (field: any) => MessageTicket.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => MessageTicket.fromJSON( json, ), fromSuiParsedData: (content: SuiParsedData) => MessageTicket.fromSuiParsedData( content, ), fromSuiObjectData: (content: SuiObjectData) => MessageTicket.fromSuiObjectData( content, ), fetch: async (client: SuiClient, id: string) => MessageTicket.fetch( client, id, ), new: ( fields: MessageTicketFields, ) => { return new MessageTicket( [], fields ) }, kind: "StructClassReified", } }
 ...
 static fromSuiObjectData( data: SuiObjectData ): MessageTicket { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isMessageTicket(data.bcs.type)) { throw new Error(`object at is not a MessageTicket object`); }

 return MessageTicket.fromBcs( fromB64(data.bcs.bcsBytes) ); } if (data.content) { return MessageTicket.fromSuiParsedData( data.content ) } throw new Error( "Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request." ); }
