...
export function isGovernanceWitness(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V1}::upgrade_contract::GovernanceWitness`; }
...
export class GovernanceWitness implements StructClass { __StructClass = true as const;
 ...
 static readonly $typeName = `${PKG_V1}::upgrade_contract::GovernanceWitness`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

 readonly $typeName = GovernanceWitness.$typeName; readonly $fullTypeName: `${typeof PKG_V1}::upgrade_contract::GovernanceWitness`; readonly $typeArgs: []; readonly $isPhantom = GovernanceWitness.$isPhantom;

 readonly dummyField: ToField<"bool">

 private constructor(typeArgs: [], fields: GovernanceWitnessFields, ) { this.$fullTypeName = composeSuiType( GovernanceWitness.$typeName, ...typeArgs ) as `${typeof PKG_V1}::upgrade_contract::GovernanceWitness`; this.$typeArgs = typeArgs;

 this.dummyField = fields.dummyField; }

 static reified( ): GovernanceWitnessReified { return { typeName: GovernanceWitness.$typeName, fullTypeName: composeSuiType( GovernanceWitness.$typeName, ...[] ) as `${typeof PKG_V1}::upgrade_contract::GovernanceWitness`, typeArgs: [ ] as [], isPhantom: GovernanceWitness.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => GovernanceWitness.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => GovernanceWitness.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => GovernanceWitness.fromBcs( data, ), bcs: GovernanceWitness.bcs, fromJSONField: (field: any) => GovernanceWitness.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => GovernanceWitness.fromJSON( json, ), fromSuiParsedData: (content: SuiParsedData) => GovernanceWitness.fromSuiParsedData( content, ), fromSuiObjectData: (content: SuiObjectData) => GovernanceWitness.fromSuiObjectData( content, ), fetch: async (client: SuiClient, id: string) => GovernanceWitness.fetch( client, id, ), new: ( fields: GovernanceWitnessFields, ) => { return new GovernanceWitness( [], fields ) }, kind: "StructClassReified", } }
...
export function isContractUpgraded(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V1}::upgrade_contract::ContractUpgraded`; }
...
export function isUpgradeContract(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V1}::upgrade_contract::UpgradeContract`; }
