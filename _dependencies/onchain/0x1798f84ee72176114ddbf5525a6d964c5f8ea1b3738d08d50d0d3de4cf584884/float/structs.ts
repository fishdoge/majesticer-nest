...
export class Float implements StructClass { __StructClass = true as const;
 ...
 static reified( ): FloatReified { return { typeName: Float.$typeName, fullTypeName: composeSuiType( Float.$typeName, ...[] ) as `${typeof PKG_V1}::float::Float`, typeArgs: [ ] as [], isPhantom: Float.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => Float.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => Float.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => Float.fromBcs( data, ), bcs: Float.bcs, fromJSONField: (field: any) => Float.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => Float.fromJSON( json, ), fromSuiParsedData: (content: SuiParsedData) => Float.fromSuiParsedData( content, ), fromSuiObjectData: (content: SuiObjectData) => Float.fromSuiObjectData( content, ), fetch: async (client: SuiClient, id: string) => Float.fetch( client, id, ), new: ( fields: FloatFields, ) => { return new Float( [], fields ) }, kind: "StructClassReified", } }
