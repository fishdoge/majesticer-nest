...
export function isPriceInfoObject(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V1}::price_info::PriceInfoObject`; }
...
export class PriceInfoObject implements StructClass { __StructClass = true as const;

 static readonly $typeName = `${PKG_V1}::price_info::PriceInfoObject`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

 readonly $typeName = PriceInfoObject.$typeName; readonly $fullTypeName: `${typeof PKG_V1}::price_info::PriceInfoObject`; readonly $typeArgs: []; readonly $isPhantom = PriceInfoObject.$isPhantom;

 readonly id: ToField<UID>; readonly priceInfo: ToField<PriceInfo>

 private constructor(typeArgs: [], fields: PriceInfoObjectFields, ) { this.$fullTypeName = composeSuiType( PriceInfoObject.$typeName, ...typeArgs ) as `${typeof PKG_V1}::price_info::PriceInfoObject`; this.$typeArgs = typeArgs;

 this.id = fields.id;; this.priceInfo = fields.priceInfo; }

 static reified( ): PriceInfoObjectReified { return { typeName: PriceInfoObject.$typeName, fullTypeName: composeSuiType( PriceInfoObject.$typeName, ...[] ) as `${typeof PKG_V1}::price_info::PriceInfoObject`, typeArgs: [ ] as [], isPhantom: PriceInfoObject.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => PriceInfoObject.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => PriceInfoObject.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => PriceInfoObject.fromBcs( data, ), bcs: PriceInfoObject.bcs, fromJSONField: (field: any) => PriceInfoObject.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => PriceInfoObject.fromJSON( json, ), fromSuiParsedData: (content: SuiParsedData) => PriceInfoObject.fromSuiParsedData( content, ), fromSuiObjectData: (content: SuiObjectData) => PriceInfoObject.fromSuiObjectData( content, ), fetch: async (client: SuiClient, id: string) => PriceInfoObject.fetch( client, id, ), new: ( fields: PriceInfoObjectFields, ) => { return new PriceInfoObject( [], fields ) }, kind: "StructClassReified", } }
 ...
 static fromSuiObjectData( data: SuiObjectData ): PriceInfoObject { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isPriceInfoObject(data.bcs.type)) { throw new Error(`object at is not a PriceInfoObject object`); }

 return PriceInfoObject.fromBcs( fromB64(data.bcs.bcsBytes) ); } if (data.content) { return PriceInfoObject.fromSuiParsedData( data.content ) } throw new Error( "Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request." ); }
...
export function isPriceInfo(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V1}::price_info::PriceInfo`; }
