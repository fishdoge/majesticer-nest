...
export function isJob(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V1}::job::Job`; }
...
export class Job implements StructClass { __StructClass = true as const;

 static readonly $typeName = `${PKG_V1}::job::Job`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

 readonly $typeName = Job.$typeName; readonly $fullTypeName: `${typeof PKG_V1}::job::Job`; readonly $typeArgs: []; readonly $isPhantom = Job.$isPhantom;
 ...
 private constructor(typeArgs: [], fields: JobFields, ) { this.$fullTypeName = composeSuiType( Job.$typeName, ...typeArgs ) as `${typeof PKG_V1}::job::Job`; this.$typeArgs = typeArgs;

 this.id = fields.id;; this.name = fields.name;; this.hash = fields.hash;; this.data = fields.data;; this.createdAt = fields.createdAt; }

 static reified( ): JobReified { return { typeName: Job.$typeName, fullTypeName: composeSuiType( Job.$typeName, ...[] ) as `${typeof PKG_V1}::job::Job`, typeArgs: [ ] as [], isPhantom: Job.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => Job.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => Job.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => Job.fromBcs( data, ), bcs: Job.bcs, fromJSONField: (field: any) => Job.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => Job.fromJSON( json, ), fromSuiParsedData: (content: SuiParsedData) => Job.fromSuiParsedData( content, ), fromSuiObjectData: (content: SuiObjectData) => Job.fromSuiObjectData( content, ), fetch: async (client: SuiClient, id: string) => Job.fetch( client, id, ), new: ( fields: JobFields, ) => { return new Job( [], fields ) }, kind: "StructClassReified", } }
 ...
 static fromSuiObjectData( data: SuiObjectData ): Job { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isJob(data.bcs.type)) { throw new Error(`object at is not a Job object`); }

 return Job.fromBcs( fromB64(data.bcs.bcsBytes) ); } if (data.content) { return Job.fromSuiParsedData( data.content ) } throw new Error( "Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request." ); }
